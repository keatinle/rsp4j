...
public class ContinuousQueryExecutionObserverImpl<I, W, R, O> extends ContinuousQueryExecutionObserver<I, W, R, O> {

    private static final Logger log = Logger.getLogger(ContinuousQueryExecutionObserverImpl.class);
    ...
    public ContinuousQueryExecutionObserverImpl(SDS sds, ContinuousQuery query, DataStream<O> outstream, RelationToRelationOperator<W, R> r2r, RelationToStreamOperator<R, O> r2s, StreamToRelationOp<I, W>... s2rs) {
        super(sds, query);
    ...
    @Override
    public void update(Observable s2r, Object timestamp) {
        Long now = (Long) timestamp;
